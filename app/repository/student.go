...
// Daftar putih kolom untuk mencegah SQL injection pada klausa ORDER BY
...
// buildFilter menyusun klausa WHERE dinamis menggunakan argumen terparameter ($1, $2, dst)
...
func (r *studentPostgresRepository) FindAll(ctx context.Context, q model.ListQuery) ([]model.Student, int, error) {
	...
	// 1. Hitung total baris yang cocok dengan filter
	var total int
	...
	// 2. Ambil data dengan sorting dan pagination di database
	sortCol, ok := kolomUrut[q.Sort]
	if !ok {
		sortCol = "id"
	}
...
// isUniqueViolation mendeteksi error duplicate key PostgreSQL (kode 23505)
